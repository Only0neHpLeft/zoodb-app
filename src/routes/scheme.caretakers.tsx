    ...
    const loadData = async () => {
      try {
        const result = isCzech
          ? await executeQuery(
              'SELECT id, jmeno, narozen FROM osetrovatele ORDER BY id',
            )
          : await executeQuery(
              'SELECT id, name, born FROM caretakers ORDER BY id',
            )

        const mappedCaretakers = (result.rows as Record<string, unknown>[]).map(
          (row) => ({
            id: row.id as number,
            ...
            <>
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="text-2xl font-bold">
                      {t.pages.caretakers.title}
                    </h2>
                  </div>
                  <p className="text-muted-foreground">
                    {t.pages.caretakers.description}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {isCzech ? 'Zobrazeno' : 'Showing'}{' '}
                  {displayedCaretakers.length} {t.pages.caretakers.of}{' '}
                  {filteredCaretakers.length}{' '}
                  {t.pages.caretakers.caretakersCount}
                </div>
              </div>
                          ...
                          <Empty className="border-0">
                            <EmptyHeader>
                              ...
                              <EmptyTitle>
                                {searchQuery
                                  ? isCzech
                                    ? 'Žádní ošetřovatelé nenalezeni'
                                    : 'No caretakers found'
                                  : isCzech
                                    ? 'Zatím žádní ošetřovatelé'
                                    : 'No caretakers yet'}
                              </EmptyTitle>
                              <EmptyDescription>
                                {searchQuery
              ...
              {hasMore && (
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {t.pages.caretakers.showMore} (+
                    {Math.min(PAGE_SIZE, remaining)}{' '}
                    {t.pages.caretakers.remaining})
                  </Button>
                </div>
              )}
            </>
