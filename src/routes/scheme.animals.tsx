...
function AnimalsPage() {
  ...
  const getTypeName = (typeId: number): string =>
    types.find((t) => t.id === typeId)?.title || `Type ${typeId}`
      ...
      try {
        ...
        if (isCzech) {
          ;[animalsResult, typesResult] = await Promise.all([
            executeQuery(
              'SELECT id, druh, jmeno, vaha, narozen, spotreba FROM zvirata ORDER BY id',
            ),
          ...
          ;[animalsResult, typesResult] = await Promise.all([
            executeQuery(
              'SELECT id, type, name, weight, born, consumption FROM animals ORDER BY id',
            ),
        ...
        const mappedAnimals = (
          animalsResult.rows as Record<string, unknown>[]
        ).map((row) => ({
          id: row.id as number,
          ...
          consumption:
            (isCzech ? row.spotreba : row.consumption) !== null
              ? Number(isCzech ? row.spotreba : row.consumption)
              : null,
        }))
            ...
            <>
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {t.pages.animals.showing} {displayedAnimals.length}{' '}
                  {t.pages.animals.of} {filteredAnimals.length}{' '}
                  {t.pages.animals.animalsCount}
                </div>
              </div>
                    ...
                    {filteredAnimals.length === 0 ? (
                        ...
                        <TableCell colSpan={6}>
                          <Empty className="border-0">
                            <EmptyHeader>
                              ...
                              <EmptyTitle>
                                {searchQuery
                                  ? t.pages.animals.noAnimalsFound
                                  : t.pages.animals.noAnimalsYet}
                              </EmptyTitle>
                              <EmptyDescription>
                                {searchQuery
                                  ? t.pages.animals.clearSearch
                                  : t.pages.animals.addDescription}
                              </EmptyDescription>
                            </EmptyHeader>
                            {searchQuery && (
                              <EmptyContent>
                      ...
                      displayedAnimals.map((animal) => (
                        <TableRow key={animal.id}>
                          ...
                          <TableCell className="text-right tabular-nums font-mono text-xs">
                            {animal.consumption !== null ? (
                              animal.consumption.toLocaleString()
                            ) : (
              ...
              {hasMore && (
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {t.pages.animals.showMore} (+
                    {Math.min(PAGE_SIZE, remaining)} {t.pages.animals.remaining}
                    )
                  </Button>
                </div>
              )}
