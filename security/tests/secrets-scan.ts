...
import type { Finding, TestModule, RunOptions, TestResult, Severity } from "../types";
...
function walkDir(dir: string): string[] {
  ...
  try {
    ...
    const output = execSync(`ls -1a "${dir}"`, { encoding: "utf8", stdio: ["pipe", "pipe", "pipe"] });
...
async function checkFilesForSecrets(
  ...
  for (const filePath of allFiles) {
    ...
    if (relPath === THIS_FILE || relPath.replace(/\\/g, "/") === THIS_FILE) continue;
