...
type Like = { id: number; caretakerId: number; typeId: number; caretakerName?: string; typeName?: string }
...
function LikesPage() {
      ...
      try {
        ...
        if (isCzech) {
          [likesResult, caretakersResult, typesResult] = await Promise.all([
            executeQuery("SELECT id, osetrovatel, druh FROM ma_rad ORDER BY id"),
  ...
  const filteredLikes = filterLikes(likes, searchQuery, isCzech, getCaretakerName, getTypeName)
            ...
            <>
              <div className="flex items-center justify-between">
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="text-2xl font-bold">{t.pages.likes.title}</h2>
                  </div>
                  <p className="text-muted-foreground">{t.pages.likes.description}</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {isCzech ? "Zobrazeno" : "Showing"} {displayedLikes.length} {t.pages.likes.of} {filteredLikes.length} {t.pages.likes.likesCount}
                </div>
              </div>
                    ...
                    {filteredLikes.length === 0 ? (
                      ...
                      displayedLikes.map((like) => (
                        <TableRow key={like.id}>
                          ...
                          <TableCell>{getCaretakerName(like.caretakerId)}</TableCell>
                          <TableCell className="capitalize">{getTypeName(like.typeId)}</TableCell>
                        </TableRow>
                      ))
                    )}
                  </TableBody>
              ...
              {hasMore && (
                <div className="flex justify-center pt-4">
                  <Button variant="outline" onClick={() => setDisplayLimit(prev => prev + PAGE_SIZE)}>
                    {t.pages.likes.showMore} (+{Math.min(PAGE_SIZE, remaining)} {t.pages.likes.remaining})
                  </Button>
                </div>
              )}
            </>
