    ...
    const loadData = async () => {
      try {
        const result = isCzech
          ? await executeQuery(
              'SELECT id, nazev, vaha_min, vaha_max FROM druhy ORDER BY id',
            )
          : await executeQuery(
              'SELECT id, name, weight_min, weight_max FROM types ORDER BY id',
            )

        const mappedTypes = (result.rows as Record<string, unknown>[]).map(
          (row) => ({
            id: row.id as number,
            ...
            <>
              ...
              <div className="flex items-center gap-4">
                <div className="relative flex-1 max-w-sm flex items-center gap-2">
                  <div className="relative flex-1">
                    ...
                    <Input
                      placeholder={
                        isCzech ? 'Hledat druhy...' : 'Search types...'
                      }
                      value={searchQuery}
                ...
                <div className="text-sm text-muted-foreground">
                  {isCzech ? 'Zobrazeno' : 'Showing'} {displayedTypes.length}{' '}
                  {t.pages.types.of} {filteredTypes.length}{' '}
                  {t.pages.types.typesCount}
                </div>
              </div>
                      ...
                      displayedTypes.map((type) => (
                        <TableRow key={type.id}>
                          ...
                          <TableCell className="text-right tabular-nums font-mono text-xs">
                            {type.weightMin.toFixed(1)}
                          </TableCell>
                          <TableCell className="text-right tabular-nums font-mono text-xs">
                            {type.weightMax.toFixed(1)}
                          </TableCell>
                        </TableRow>
                      ))
                    )}
              ...
              {hasMore && (
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {t.pages.types.showMore} (+{Math.min(PAGE_SIZE, remaining)}{' '}
                    {t.pages.types.remaining})
                  </Button>
                </div>
              )}
            </>
