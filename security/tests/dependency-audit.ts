...
interface NpmAuditVulnerability {
  ...
  readonly url?: string;
  readonly fixAvailable?: boolean | { readonly name: string; readonly version: string };
  readonly via?: ReadonlyArray<{ readonly title?: string; readonly url?: string } | string>;
}
...
function runNpmAudit(projectRoot: string): AuditResult {
  ...
  if (meta) {
    const severityBuckets: ReadonlyArray<keyof NpmAuditMetadataVulnerabilities> = [
      "critical",
      "high",
      "moderate",
      "low",
...
function runBunFallback(projectRoot: string): AuditResult {
  ...
  const infoFinding: Finding = {
    ...
    impact: "Unknown — dependency vulnerabilities cannot be assessed without npm audit",
