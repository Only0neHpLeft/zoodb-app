...
function TreatsPage() {
      ...
      try {
        ...
        if (isCzech) {
          ;[treatsResult, caretakersResult, animalsResult] = await Promise.all([
            executeQuery(
              'SELECT id, osetrovatel, zvire FROM osetruje ORDER BY id',
            ),
          ...
          ;[treatsResult, caretakersResult, animalsResult] = await Promise.all([
            executeQuery(
              'SELECT id, caretaker, animal FROM treats ORDER BY id',
            ),
        ...
        const mappedTreats = (
          treatsResult.rows as Record<string, unknown>[]
        ).map((row) => ({
          id: row.id as number,
  ...
  const getAnimalName = (id: number) =>
    animals.find((a) => a.id === id)?.name || `ID: ${id}`

  const filteredTreats = filterTreats(
    treats,
    searchQuery,
    isCzech,
    getCaretakerName,
            ...
            <>
                ...
                <div>
                  <div className="flex items-center gap-3">
                    <h2 className="text-2xl font-bold">
                      {t.pages.treats.title}
                    </h2>
                  </div>
                  <p className="text-muted-foreground">
                    {t.pages.treats.description}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                ...
                <div className="text-sm text-muted-foreground">
                  {isCzech ? 'Zobrazeno' : 'Showing'} {displayedTreats.length}{' '}
                  {t.pages.treats.of} {filteredTreats.length}{' '}
                  {t.pages.treats.treatsCount}
                </div>
              </div>
                    ...
                    {filteredTreats.length === 0 ? (
                      ...
                      displayedTreats.map((treat) => (
                        <TableRow key={treat.id}>
                          ...
                          <TableCell>
                            {getCaretakerName(treat.caretakerId)}
                          </TableCell>
              ...
              {hasMore && (
                <div className="flex justify-center pt-4">
                  ...
                  >
                    {t.pages.treats.showMore} (+{Math.min(PAGE_SIZE, remaining)}{' '}
                    {t.pages.treats.remaining})
                  </Button>
                </div>
              )}
            </>
