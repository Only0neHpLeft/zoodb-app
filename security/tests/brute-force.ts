...
const FAILED_ATTEMPT_LOG_PATTERNS = [
  'console.log',
  'console.warn',
  'log',
  'attempt',
...
function checkCodeGenSecurity(source: string): CodeGenCheckResult {
  ...
  const finding: Finding = {
    ...
    remediation: 'Use crypto.randomUUID().substring(0, 8) or a similar CSPRNG',
  }
...
function checkJoinClassRateLimiting(source: string): RateLimitCheckResult {
  ...
  const hasRateLimit = RATE_LIMIT_PATTERNS.some((pattern) =>
  ...
  const hasLogCall =
    body.includes('console.log') || body.includes('console.warn')
